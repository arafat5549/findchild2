...
CREATE DATABASE IF NOT EXISTS  `xunqin`;   -- 加入双斜引号 避免你的数据库名和表名等跟数据库的关键字重名
USE `xunqin`;   -- 选择数据库
...
CREATE TABLE `sys_user` (
  ...
  `type`     int(5) DEFAULT 1 COMMENT '用户类型 1普通用户 2管理员',
...
CREATE TABLE `sys_news` (
  ...
  `from` varchar(150) DEFAULT NULL COMMENT '摘抄自',
...
CREATE TABLE `sys_announce` (
  ...
  `user_id` varchar(50) DEFAULT NULL COMMENT '公告发布人',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY  (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `sys_xunqin` (
  ...
  `losttime` datetime DEFAULT NULL COMMENT '登记年龄',
