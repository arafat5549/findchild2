...
public class AdminNewsController extends HttpServlet{
	...
	public static final String VIEW_PATH = "/WEB-INF/views/admin/news/";

	private void listAction(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException{
		req.setAttribute("title", "新闻管理");
		List<NewsType> newsTypes = Globals.findNewsTypes();
	...
	private void listRedirectAction(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException{
		resp.sendRedirect(req.getContextPath()+"/admin/news?method=list");
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if("list".equals(method)){ 
			...
			List<News> lists = newsService.findAll();
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if("add".equals(method))
		{
			...
			boolean flag = newsService.save(record);
			if(flag){
				listRedirectAction(req, resp);
			}
			else{
			...
			int id = Integer.parseInt(req.getParameter("id"));
			News record = newsService.findById(id);
			if(record == null){
				req.setAttribute(Globals.ERROR_MESSAGE, ResultEnum.INVALID_NEWS);
				listAction(req,resp);
				return;
			}
			String title = req.getParameter("title");
			...
			record.setContent(content);
			record.setType(type);
			boolean flag = newsService.update(record);
			if(flag){
				listRedirectAction(req, resp);
			}
			else{
